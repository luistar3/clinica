...
class business_Rol
{
	...
	public function fncBusinessVerificarExistenciaUsuario($usuario){
		...
		if ($connectionstatus) 
		{
			$sql = "usp_Clinica_Usuario_verificarExistenciaUsuario";
			...
			mssql_bind($proc, '@usuario', $usuario, SQLVARCHAR, false, false, 10);
...
public function fncBusinessInsertarUsuario($idRol,$usuario,$contrasena,$nombre,$idpersona){
	...
	if ($connectionstatus) 
	{
		$sql = "usp_Clinica_Usuario_insertarUsuario";
				//$USRId = $_SESSION['usuario']["ses_USRId"] ;
				// echo "usp_Sed_S_Egresado_Consultar ".$USRId.', '.$idPtaDependenciaFijo.', '.$NombreApellido.', '.$varDni.', '.$intEdad.', '.$IdGradoAcademico.', '.$IdSectorAcademico;
		$proc = mssql_init($sql, $connectionstatus); 
		mssql_bind($proc, '@idRol', $idRol, SQLINT4, false, false, 10);
		mssql_bind($proc, '@usuario', $usuario, SQLVARCHAR, false, false, 10); 
		mssql_bind($proc, '@constrasena', $contrasena, SQLVARCHAR, false, false, 10); 
		mssql_bind($proc, '@nombre', $nombre, SQLVARCHAR, false, false, 10); 
  ...
  public function fncBusinessListarRol(){
	...
	public function fncBusinessListarPersonasSinUsuario(){
		...
		if ($connectionstatus) 
		{
			$sql = "usp_Clinica_Usuario_ListarPersonaSinUsuario";
					//$USRId = $_SESSION['usuario']["ses_USRId"] ;
					// echo "usp_Sed_S_Egresado_Consultar ".$USRId.', '.$idPtaDependenciaFijo.', '.$NombreApellido.', '.$varDni.', '.$intEdad.', '.$IdGradoAcademico.', '.$IdSectorAcademico;
			$proc = mssql_init($sql, $connectionstatus); 
	...
	public function fncBusinessListarUsuario(){
		...
		if ($connectionstatus) 
		{
			$sql = "usp_Clinica_Usuario_listarUsuario";
					//$USRId = $_SESSION['usuario']["ses_USRId"] ;
					// echo "usp_Sed_S_Egresado_Consultar ".$USRId.', '.$idPtaDependenciaFijo.', '.$NombreApellido.', '.$varDni.', '.$intEdad.', '.$IdGradoAcademico.', '.$IdSectorAcademico;
			$proc = mssql_init($sql, $connectionstatus); 
						// mssql_bind($proc, '@USRId', $USRId, SQLINT4, false, false, 10);
